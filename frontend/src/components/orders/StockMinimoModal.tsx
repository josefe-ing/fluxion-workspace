...
export default function StockMinimoModal({ isOpen, onClose, producto, stockParams }: StockMinimoModalProps) {
  ...
  if (ventaDiariaBultos >= ABC_THRESHOLDS.A) {
    clasificacion = 'A';
    multiplicador = stockParams.stock_min_mult_a;
  } else if (ventaDiariaBultos >= ABC_THRESHOLDS.AB) {
    clasificacion = 'AB';
    multiplicador = stockParams.stock_min_mult_ab;
  } else if (ventaDiariaBultos >= ABC_THRESHOLDS.B) {
    clasificacion = 'B';
    multiplicador = stockParams.stock_min_mult_b;
  } else if (ventaDiariaBultos >= ABC_THRESHOLDS.BC) {
    clasificacion = 'BC';
    multiplicador = stockParams.stock_min_mult_bc;
  } else if (ventaDiariaBultos >= ABC_THRESHOLDS.C) {
    clasificacion = 'C';
    multiplicador = stockParams.stock_min_mult_c;
  } else {
    clasificacion = '-';
    multiplicador = 0;
  }

  const stockMinimoBultos = ventaDiariaBultos * multiplicador;
  const diasCobertura = multiplicador;
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-bold text-gray-900">
            Stock Mínimo: {diasCobertura} días ({stockMinimoBultos.toFixed(1)} bultos)
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gradient-to-r from-orange-50 to-amber-50 border-2 border-orange-300 rounded-lg p-5">
            <h3 className="text-sm font-semibold text-orange-900 mb-3">Stock Mínimo Calculado</h3>
            <div className="text-center mb-4">
              <p className="text-xs text-gray-600 mb-2">Días de cobertura</p>
              <p className="text-5xl font-bold text-orange-700">{diasCobertura} días</p>
              <p className="text-xl text-orange-600 mt-2">({stockMinimoBultos.toFixed(2)} bultos)</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {/* Paso 3: Multiplicador */}
            <div className="bg-blue-50 rounded-lg p-4 border-l-4 border-blue-500">
              <p className="text-sm font-semibold text-gray-900 mb-2">Paso 3: Aplicar Multiplicador de Stock Mínimo</p>
              <div className="space-y-3">
                <p className="text-sm text-gray-700">
                  Para productos de clase <span className="font-bold">{clasificacion}</span>,
                  el multiplicador configurado es:
                </p>
                <div className="bg-white rounded p-3 border border-blue-200">
                  ...
                  <p className="text-xs text-gray-600 text-center mt-1">
                    (equivalente a {multiplicador} días de cobertura)
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-green-50 rounded-lg p-4 border-l-4 border-green-500">
              <p className="text-sm font-semibold text-gray-900 mb-2">Paso 4: Calcular Stock Mínimo</p>
              <div className="space-y-2">
                <p className="text-sm text-gray-600">Fórmula:</p>
                <div className="bg-white rounded p-3 border border-green-200 font-mono text-sm">
                  <p className="text-gray-700">Stock Mínimo = Venta Diaria × Multiplicador</p>
                  <p className="text-gray-700 mt-2">
                    = {ventaDiariaBultos.toFixed(3)} bultos/día × {multiplicador}
                  </p>
                  <p className="text-lg font-bold text-green-700 mt-2 pt-2 border-t border-green-200">
                    = {stockMinimoBultos.toFixed(2)} bultos
                  </p>
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-center font-semibold text-gray-700">Multiplicador</th>
                    <th className="px-4 py-2 text-center font-semibold text-gray-700">Días Cobertura</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  <tr className={clasificacion === 'A' ? 'bg-red-50 font-bold' : ''}>
                    ...
                    <td className="px-4 py-2 text-center font-mono font-bold text-blue-700">{stockParams.stock_min_mult_a}x</td>
                    <td className="px-4 py-2 text-center text-gray-700">{stockParams.stock_min_mult_a} días</td>
                  </tr>
                  <tr className={clasificacion === 'AB' ? 'bg-orange-50 font-bold' : ''}>
                    ...
                    <td className="px-4 py-2 text-center font-mono font-bold text-blue-700">{stockParams.stock_min_mult_ab}x</td>
                    <td className="px-4 py-2 text-center text-gray-700">{stockParams.stock_min_mult_ab} días</td>
                  </tr>
                  <tr className={clasificacion === 'B' ? 'bg-yellow-50 font-bold' : ''}>
                    ...
                    <td className="px-4 py-2 text-center font-mono font-bold text-blue-700">{stockParams.stock_min_mult_b}x</td>
                    <td className="px-4 py-2 text-center text-gray-700">{stockParams.stock_min_mult_b} días</td>
                  </tr>
                  <tr className={clasificacion === 'BC' ? 'bg-amber-50 font-bold' : ''}>
                    ...
                    <td className="px-4 py-2 text-center font-mono font-bold text-blue-700">{stockParams.stock_min_mult_bc}x</td>
                    <td className="px-4 py-2 text-center text-gray-700">{stockParams.stock_min_mult_bc} días</td>
                  </tr>
                  <tr className={clasificacion === 'C' ? 'bg-gray-50 font-bold' : ''}>
                    ...
                    <td className="px-4 py-2 text-center font-mono font-bold text-blue-700">{stockParams.stock_min_mult_c}x</td>
                    <td className="px-4 py-2 text-center text-gray-700">{stockParams.stock_min_mult_c} días</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-700 leading-relaxed">
              El <strong>Stock Mínimo</strong> es la cantidad mínima de inventario que debe mantenerse
              para evitar quiebres de stock. Se calcula multiplicando la venta diaria por un factor
              que depende de la clasificación ABC del producto. Los productos de alta rotación (clase A)
              tienen multiplicadores más bajos porque se reponen con mayor frecuencia, mientras que los
              de baja rotación (clase C) tienen multiplicadores más altos para compensar ciclos de
              reposición más largos.
            </p>
          </div>
        </div>
