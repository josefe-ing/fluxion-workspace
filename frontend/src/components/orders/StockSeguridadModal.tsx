...
export default function StockSeguridadModal({ isOpen, onClose, producto, stockParams }: StockSeguridadModalProps) {
  ...
  if (ventaDiariaBultos >= ABC_THRESHOLDS.A) {
    clasificacion = 'A';
    multiplicador = stockParams.stock_seg_mult_a;
  } else if (ventaDiariaBultos >= ABC_THRESHOLDS.AB) {
    clasificacion = 'AB';
    multiplicador = stockParams.stock_seg_mult_ab;
  } else if (ventaDiariaBultos >= ABC_THRESHOLDS.B) {
    clasificacion = 'B';
    multiplicador = stockParams.stock_seg_mult_b;
  } else if (ventaDiariaBultos >= ABC_THRESHOLDS.BC) {
    clasificacion = 'BC';
    multiplicador = stockParams.stock_seg_mult_bc;
  } else if (ventaDiariaBultos >= ABC_THRESHOLDS.C) {
    clasificacion = 'C';
    multiplicador = stockParams.stock_seg_mult_c;
  } else {
    clasificacion = '-';
    multiplicador = 0;
  }

  const stockSeguridadBultos = ventaDiariaBultos * multiplicador;
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-bold text-gray-900">
            Stock de Seguridad: {diasCobertura} días ({stockSeguridadBultos.toFixed(1)} bultos)
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-cyan-50 border-2 border-blue-300 rounded-lg p-5">
            <h3 className="text-sm font-semibold text-blue-900 mb-3">Stock de Seguridad Calculado</h3>
            <div className="text-center mb-4">
              <p className="text-xs text-gray-600 mb-2">Días de cobertura</p>
              <p className="text-5xl font-bold text-blue-700">{diasCobertura} días</p>
              <p className="text-xl text-blue-600 mt-2">({stockSeguridadBultos.toFixed(2)} bultos)</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-blue-50 rounded-lg p-4 border-l-4 border-blue-500">
              <p className="text-sm font-semibold text-gray-900 mb-2">Paso 3: Aplicar Multiplicador de Stock de Seguridad</p>
            ...
            <div className="bg-green-50 rounded-lg p-4 border-l-4 border-green-500">
              ...
              <div className="space-y-2">
                ...
                <div className="bg-white rounded p-3 border border-green-200 font-mono text-sm">
                  <p className="text-gray-700">Stock Seguridad = Venta Diaria × Multiplicador</p>
                  ...
                  <p className="text-lg font-bold text-green-700 mt-2 pt-2 border-t border-green-200">
                    = {stockSeguridadBultos.toFixed(2)} bultos
                  </p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-200">
                  <tr className={clasificacion === 'A' ? 'bg-red-50 font-bold' : ''}>
                    ...
                    <td className="px-4 py-2 text-center text-gray-700">{stockParams.stock_seg_mult_a} días</td>
                  </tr>
                  <tr className={clasificacion === 'AB' ? 'bg-orange-50 font-bold' : ''}>
                    ...
                    <td className="px-4 py-2 text-center text-gray-700">{stockParams.stock_seg_mult_ab} días</td>
                  </tr>
                  <tr className={clasificacion === 'B' ? 'bg-yellow-50 font-bold' : ''}>
                    ...
                    <td className="px-4 py-2 text-center text-gray-700">{stockParams.stock_seg_mult_b} días</td>
                  </tr>
                  <tr className={clasificacion === 'BC' ? 'bg-amber-50 font-bold' : ''}>
                    ...
                    <td className="px-4 py-2 text-center text-gray-700">{stockParams.stock_seg_mult_bc} días</td>
                  </tr>
                  <tr className={clasificacion === 'C' ? 'bg-gray-50 font-bold' : ''}>
                    ...
                    <td className="px-4 py-2 text-center text-gray-700">{stockParams.stock_seg_mult_c} días</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-700 leading-relaxed">
              El <strong>Stock de Seguridad</strong> es un buffer adicional de inventario que se mantiene
              para protegerse contra la variabilidad en la demanda y en los tiempos de reposición. Se calcula
              multiplicando la venta diaria por un factor que varía según la clasificación ABC del producto.
              Los productos de alta rotación (clase A) tienen multiplicadores más bajos porque se monitorean
              y reponen con mayor frecuencia, mientras que los de baja rotación (clase C) requieren mayores
              buffers de seguridad debido a ciclos de reposición más largos y mayor incertidumbre.
            </p>
          </div>
        </div>
