...
class VentasTransformer:
    ...
    def _normalizar_numeros(self, df: pd.DataFrame) -> pd.DataFrame:
        """Normaliza y valida campos numéricos"""
        ...
        for campo in campos_numericos:
            if campo in df.columns:
                ...
                # Reemplazar valores negativos o zero según el campo
                if campo in ['cantidad_vendida', 'precio_unitario']:
                    df[campo] = df[campo].where(df[campo] > 0, np.nan)
                else:
                    df[campo] = df[campo].where(df[campo] >= 0, np.nan)

        # Redondear campos monetarios a 2 decimales
    ...
    def _validar_y_limpiar(self, df: pd.DataFrame) -> pd.DataFrame:
        """Valida y limpia registros inválidos"""
        self.logger.info("🔍 Validando y limpiando datos...")

        inicial = len(df)

        # Eliminar registros con campos críticos nulos
        campos_criticos = ['numero_factura', 'codigo_producto', 'cantidad_vendida']
        for campo in campos_criticos:
            if campo in df.columns:
                ...
                if eliminados > 0:
                    self.logger.warning(f"⚠️ Eliminados {eliminados} registros por {campo} nulo")
        ...
        if 'cantidad_vendida' in df.columns:
            ...
            eliminados = antes - len(df)
            if eliminados > 0:
                self.logger.warning(f"⚠️ Eliminados {eliminados} registros por cantidad inválida")
