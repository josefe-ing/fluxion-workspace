-- =========================================================================
-- Migration 014 UP: Sistema de Configuración Dinámica de Inventario
-- Description: Crea tablas de configuración para parámetros ABC y niveles
--              de servicio (migración desde DuckDB a PostgreSQL)
-- Date: 2025-12-06
-- Author: System
-- =========================================================================
...
CREATE TABLE IF NOT EXISTS config_inventario_global (
    ...
    categoria VARCHAR(50) NOT NULL,  -- 'abc_umbrales', 'xyz_umbrales', 'niveles_servicio', etc.
    parametro VARCHAR(100) NOT NULL,
    valor_numerico DECIMAL(10,4),
    valor_texto VARCHAR(255),
    descripcion VARCHAR(500),
    unidad VARCHAR(50),  -- 'bultos_dia', 'coeficiente', 'porcentaje', 'dias', 'zscore'
...
-- -------------------------------------------------------------------------
-- 2. Configuración por Tienda (parámetros ABC por tienda)
-- -------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS config_parametros_abc_tienda (
    ...
    -- Parámetros de Clase A
    clase_a_z_score DECIMAL(6,4) DEFAULT 2.33,
    clase_a_dias_cobertura INTEGER DEFAULT 7,

    -- Parámetros de Clase B
    clase_b_z_score DECIMAL(6,4) DEFAULT 1.88,
    clase_b_dias_cobertura INTEGER DEFAULT 14,

    -- Parámetros de Clase C
    clase_c_dias_cobertura INTEGER DEFAULT 30,

    -- Lead time
    lead_time_dias DECIMAL(4,2) DEFAULT 1.5,
...
-- -------------------------------------------------------------------------
-- 3. Configuración por Tienda Legacy (compatibilidad)
-- -------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS config_inventario_tienda (
    id VARCHAR(100) PRIMARY KEY,
    tienda_id VARCHAR(50) NOT NULL,
    categoria_producto VARCHAR(50) NOT NULL,  -- 'seco', 'frio', 'verde'
    clasificacion_abc VARCHAR(10) NOT NULL,   -- 'A', 'AB', 'B', 'BC', 'C'
...
-- -------------------------------------------------------------------------
-- 4. Configuración por Producto Individual (SOLO Frío/Verde)
-- -------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS config_inventario_producto (
    ...
    tienda_id VARCHAR(50) NOT NULL,
    categoria_producto VARCHAR(50) NOT NULL,  -- 'frio' o 'verde' únicamente
    ...
    dias_no_pedir VARCHAR(100),  -- Ej: 'Lunes,Miércoles'

    -- Override de clasificación ABC (opcional)
    clasificacion_abc_override VARCHAR(10),
...
-- -------------------------------------------------------------------------
-- 5. Mapeo de Productos a Categorías
-- -------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS productos_categoria (
    codigo_producto VARCHAR(50) PRIMARY KEY,
    categoria_producto VARCHAR(50) NOT NULL,  -- 'seco', 'frio', 'verde'
    subcategoria VARCHAR(100),  -- 'carniceria', 'charcuteria', 'fruver', etc.
...
CREATE INDEX IF NOT EXISTS idx_config_abc_tienda_lookup
    ON config_parametros_abc_tienda(tienda_id);

-- -------------------------------------------------------------------------
-- 7. Datos Iniciales: Configuración Global
-- -------------------------------------------------------------------------

-- Umbrales de Clasificación ABC
INSERT INTO config_inventario_global (id, categoria, parametro, valor_numerico, descripcion, unidad)
VALUES
...
ON CONFLICT (categoria, parametro) DO NOTHING;
...
INSERT INTO config_inventario_global (id, categoria, parametro, valor_texto, descripcion, unidad)
VALUES
...
INSERT INTO config_inventario_global (id, categoria, parametro, valor_numerico, descripcion, unidad)
VALUES
    ('stock_lead_time', 'stock_general', 'lead_time_dias', 1.5, 'Lead time por defecto desde CEDI', 'dias'),
...
COMMENT ON TABLE config_inventario_global IS 'Configuración global del sistema de inventario';
COMMENT ON TABLE config_parametros_abc_tienda IS 'Configuración de parámetros ABC por tienda individual';
COMMENT ON TABLE config_inventario_tienda IS 'Configuración específica por tienda y categoría de producto';
COMMENT ON TABLE config_inventario_producto IS 'Configuración individual de productos perecederos (Frío/Verde)';
COMMENT ON TABLE productos_categoria IS 'Mapeo de productos a categorías (Seco/Frío/Verde)';
